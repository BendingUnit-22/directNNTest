...
class BenignTransf:
    ...
    def gen_benign(self, X):
        ...
        #print("Benign transformations size: ", len(deltas))
        return deltas


class DataGen:
  def __init__(self, l2_nums, linf_nums, l2_rad, linf_rad, l2_light=True, linf_light=True):
    self.linf_trans = AdvTransf(light=linf_light)
    self.l2_trans = AdvTransf(infty=False, light=l2_light)
    self.benign_trans = BenignTransf()
  ...
  def mixed_gen(self, x):
     adv_deltas = self.lp_gen(x)
     benign_deltas = self.benign_gen(x)
     return adv_deltas + benign_deltas